(function(ActiveModel,undefined) {
	...
	ActiveModel.addInstanceProperty("valid",function(on , parent) {
		...
		var runBefore = true;
		if (Utils.isFunction(this.beforeValidate)){
		...
		var runAfter = true;
		if (Utils.isFunction(this.afterValidate)){
	...
	ActiveModel.addValidate = function(name, func , funcInstall) {
		...
		this.addProperty( ("validate_"+name).camelize(true) , function(fields, opts) {
			...
			if (Utils.isFunction(funcInstall)) {
...
(function(ActiveModel,undefined) {
	...
	ActiveModel.addValidate("acceptance" ,  function(field, opts) {
			...
			var message =  Utils.getText("activerecord.errors.messages.accepted");
			if (opts.message)
	...
	ActiveModel.addValidate("confirmation", function(field, opts) {
		var result = true;
		var message = Utils.getText("activerecord.errors.messages.confirmation");
		if (opts.message)
		message = opts.message;
	...
	ActiveModel.addValidate("presence", function(field, opts, parentInvocation) {
			...
			var message = Utils.getText("activerecord.errors.messages.blank");
	...
	ActiveModel.addValidate("length", function(field, opts) {
			...
			var too_long = Utils.tmpl(Utils.getText("activerecord.errors.messages.too_long"),{count: opts.maximum});
			if (opts.too_long)
				too_long = opts.too_long;
	
			var too_short = Utils.tmpl(Utils.getText("activerecord.errors.messages.too_short"),{count: opts.minimum});
			if (opts.too_short)
				too_short = opts.too_short;
	
			var wrong_length = Utils.tmpl(Utils.getText("activerecord.errors.messages.wrong_length"),{count: opts.is});
			...
			if (typeof sizeToCheck != 'number') {
				result = false;
				this.addError(field, Utils.getText("activerecord.errors.messages.invalid"));
			}
	...
	ActiveModel.addValidate("numericality", function(field, opts) {
			...
			var greater_than = Utils.tmpl(Utils.getText("activerecord.errors.messages.greater_than"),{count: opts.greater_than});
			if (opts.greater_than)
				greater_than = opts.greater_than;
	
			var equal_to = Utils.tmpl(Utils.getText("activerecord.errors.messages.equal_to"),{count: opts.equal_to});
			if (opts.equal_to)
				equal_to = opts.equal_to;
	
			var less_than = Utils.tmpl(Utils.getText("activerecord.errors.messages.less_than"),{count: opts.less_than});
			if (opts.less_than)
	...
	ActiveModel.addValidate("format",function(field, opts) {
			...
			var message =  Utils.getText("activerecord.errors.messages.invalid");
			if (opts.message)
				message = opts.message;
	...
	ActiveModel.addValidate("inclusion", function(field, opts) {
			...
			var message =  Utils.getText("activerecord.errors.messages.inclusion");
			if (opts.message)
				message = opts.message;
	...
	ActiveModel.addValidate("exclusion", function(field, opts) {
			...
			var message =  Utils.getText("activerecord.errors.messages.exclusion");
			if (opts.message)
				message = opts.message;
